...
class OfferListComponent extends React.Component {
	...
	render() {
		...
		const allowed_types = ["businessoffers", "recommendedoffers", "topoffers", "internaloffers"];
		const foundoffertype = search_params.get("offertype");
		const offertype = foundoffertype && allowed_types.includes(foundoffertype) ? foundoffertype : "businessoffers";
					...
					<antd.Card className="nector-card" style={{ padding: 0, minHeight: "10%", borderBottom: "1px solid #eeeeee00" }} bordered={false} bodyStyle={{ padding: 20 }}>
						...
						<div style={{ display: "flex", flex: 1, alignItems: "center", marginBottom: 20 }}>
							<div style={{ display: "flex", flex: 1 }}>
								<b className="nector-title" style={{ color: "#000" }}>{offertype_info.title}</b>
							</div>
						</div>
