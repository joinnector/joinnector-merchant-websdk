...
class NotificationListComponent extends React.Component {
	...
	shouldComponentUpdate(nextProps, nextState) {
		if (nextProps.lead._id != this.props.lead._id) {
			this.api_merchant_list_notifications({ page: 1, limit: 10, lead_id: nextProps.lead._id });
		}
	...
	on_refresh(force = false) {
		...
		if (collection_helper.validate_is_null_or_undefined(this.props.notifications) === true
			...
			|| (collection_helper.validate_not_null_or_undefined(this.props.notifications.items) === true && this.props.notifications.items.length < 1)) {
			this.api_merchant_list_notifications({ page: 1, limit: 10 });
		}
	}
		...
		const render_load_more = () => {
			if (!this.state.loading) {
				...
				return (<div style={{ textAlign: "center", padding: "2%" }}>
					<antd.Button onClick={() => this.api_merchant_list_notifications({ page: Math.floor(Number(data_source.length) / this.state.limit) + 1, append_data: true })}>Load more</antd.Button>
				</div>);
			} else {
				return <div />;
			}
				...
				<div>
					<antd.Card className="nector-card" style={{ padding: 0, minHeight: "10%", backgroundColor: default_search_params.toolbar_background_color, backgroundImage: default_search_params.toolbar_background_image }} bordered={false}>
						<div style={{ position: "absolute", bottom: 0, right: 0, padding: 14, textAlign: "end" }}>
							<img src={"https://cdn.nector.io/nector-static/illustration/illustration_happy_news.png"} height={"50%"} width={"50%"} />
						</div>
