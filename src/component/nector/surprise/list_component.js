...
//from app
class SurpriseListComponent extends React.Component {
	constructor(props) {
		super(props);
		...
		this.api_merchant_list_surprises = this.api_merchant_list_surprises.bind(this);

		this.process_list_data = this.process_list_data.bind(this);

		this.on_refresh = this.on_refresh.bind(this);

		this.on_surprise = this.on_surprise.bind(this);
	...
	shouldComponentUpdate(nextProps, nextState) {
		if (nextProps.lead._id != this.props.lead._id) {
			this.api_merchant_list_surprises({ page: 1, limit: 10 });
		}
	...
	api_merchant_list_surprises(values) {
		...
		// eslint-disable-next-line no-unused-vars
		const opts = {
			event: constant_helper.get_app_constant().API_MERCHANT_LIST_SURPRISE_DISPATCH,
	...
	process_list_data() {
		return (this.props.surprises && this.props.surprises.items || []).map(item => ({ ...item, key: item._id }));
	}
	...
	on_refresh(force = false) {
		...
		if (collection_helper.validate_is_null_or_undefined(this.props.surprises) === true
			|| collection_helper.validate_is_null_or_undefined(this.props.surprises.items) === true
			|| (collection_helper.validate_not_null_or_undefined(this.props.surprises.items) === true && this.props.surprises.items.length < 1)) {
			this.api_merchant_list_surprises({ page: 1, limit: 10 });
		}
	}
	...
	on_surprise(record) {
		...
		this.props.app_action.internal_generic_dispatch(opts, (result) => {
			...
			search_params.set("surprise_id", record._id);
			this.props.history.push(`/nector/surpriseactivity-list?${search_params.toString()}`);
		});
	}
	...
	render() {
		...
		const count = (this.props.surprises && this.props.surprises.count || 0);
		...
		const render_load_more = () => {
			if (!this.state.loading) {
				...
				return (<div style={{ textAlign: "center", padding: "2%" }}>
					<antd.Button onClick={() => this.api_merchant_list_surprises({ page: Math.floor(Number(data_source.length) / this.state.limit) + 1, append_data: true })}>Load more</antd.Button>
				</div>);
			} else {
				return <div />;
			}
			...
			<ReactPullToRefresh
				...
				<div>
					<antd.Card className="nector-card" style={{ padding: 0, minHeight: "10%", backgroundColor: default_search_params.toolbar_background_color, backgroundImage: default_search_params.toolbar_background_image }} bordered={false}>
						...
						<antd.Typography.Title style={{ fontSize: "1.5em", color: default_search_params.toolbar_color }}>Surprises</antd.Typography.Title>
					</antd.Card>
					...
					<antd.Layout>
						<antd.List
							...
							renderItem={(item) => render_list_item(item, { ...this.props, on_surprise: this.on_surprise })}
						/>
					</antd.Layout>
				</div>
			</ReactPullToRefresh>
...
SurpriseListComponent.propTypes = properties;

export default SurpriseListComponent;